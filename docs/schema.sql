...
--
-- 작성일: 2026-08-09
-- 최종 수정: 2026-08-14 (preset_formats_v 뷰 추가)
--
...
-- 사후에 스키마를 덤프해 정리했습니다.
--
-- 테이블 정의는 이 파일에, 함수·트리거·인덱스·Storage 정책은 supabase/*.sql 에 둡니다.
...
CREATE TABLE public.profiles (
  ...
  avatar_url text,
  role text NOT NULL DEFAULT 'user'::text,          -- 'user' | 'admin' (CHECK 제약은 없습니다)
...
-- AI 대화 생성 이력.
-- Supabase Edge Function "generate" 가 기록합니다.
-- 비로그인 생성은 user_id 를 비워 두고, 로그인 후 본인 것으로 claim 합니다.
CREATE TABLE public.generations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid,                                      -- 비로그인 생성은 null
  preset_id smallint,
...
CREATE TABLE public.saved_contents (
  ...
  -- 저장은 이 조합으로 upsert 하므로 제약을 지우면 중복이 쌓입니다.
  CONSTRAINT saved_contents_user_id_generation_item_id_key
    UNIQUE (user_id, generation_item_id)
);

-- AI 없이 제공하는 기본 콘텐츠.
-- 카드 뒤집기(콘텐츠 카드·조커 벌칙)와 랜덤 픽에서 사용합니다.
-- *_codes 배열로 조건 필터링이 가능합니다.
...
-- write_role 은 공지사항만 'admin' 이고 나머지는 'user' 입니다.
...
-- 댓글 좋아요. 복합 PK 라 한 사람이 한 댓글에 한 번만 누를 수 있습니다.
-- 정의는 supabase/community-comment-features.sql 에 있습니다.
...
-- 게시글·댓글 신고. 접수 → 운영진 검토 → 조치 기록 3단계만 관리합니다.
-- target_id 는 게시글이면 posts.id, 댓글이면 comments.id 를 가리킵니다.
-- 대상이 둘로 갈려 FK 를 걸 수 없어 INSERT 트리거로 존재 여부와 자기 신고를 검증합니다.
-- 정의는 supabase/community-reports.sql 에 있습니다.
CREATE TABLE public.community_reports (
  ...
  status text NOT NULL DEFAULT 'pending'::text,      -- pending | reviewing | resolved | dismissed
  moderation_action text NOT NULL DEFAULT 'none'::text,  -- none | warning | content_deleted
  reviewer_id uuid,
  review_note text,
  ...
  CONSTRAINT community_reports_reporter_id_fkey FOREIGN KEY (reporter_id)
    REFERENCES auth.users(id) ON DELETE CASCADE,
  CONSTRAINT community_reports_reviewer_id_fkey FOREIGN KEY (reviewer_id)
    REFERENCES auth.users(id) ON DELETE SET NULL,
  -- 같은 사람이 같은 대상을 두 번 신고하지 못합니다.
  ...
  CONSTRAINT community_reports_moderation_action_check
    CHECK (moderation_action = ANY (ARRAY['none'::text, 'warning'::text, 'content_deleted'::text])),
  CONSTRAINT community_reports_reason_check
    CHECK (char_length(btrim(reason)) >= 2 AND char_length(btrim(reason)) <= 300)
);
...
-- 프리셋별 콘텐츠 분포 집계.
-- 조회용이며 앱에서는 사용하지 않습니다 (2026-08-14 기준 코드 참조 없음).
--
-- 활성 상태이면서 프리셋이 지정된 default_contents 를
-- preset_code · format_code 로 묶어 건수와 level 범위를 냅니다.
-- 새로 노출하는 데이터는 없고, default_contents 의 RLS 가
-- "활성만 읽기" 라 우회 문제도 없습니다.
CREATE OR REPLACE VIEW public.preset_formats_v AS
  SELECT preset_code,
         format_code,
   ...
   WHERE is_active AND preset_code IS NOT NULL
   GROUP BY preset_code, format_code;


-- =============================================================================
-- 3. 함수
--
-- 아래는 대시보드에서 만든 초기 함수들입니다.
-- 이후 추가한 함수는 supabase/*.sql 에 정의가 있습니다. 6번 항목을 보세요.
-- =============================================================================
...
-- 생성 이력: 본인 것 + 비로그인 생성분 ----------------------------------------
-- 비로그인 사용자가 만든 행은 user_id 가 null 이고, 로그인하면 본인 것으로 claim 합니다.
-- id 가 uuid 라 남의 비로그인 결과를 주소로 찾아내기는 사실상 어렵습니다.

CREATE POLICY "본인만" ON public.generations
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "anon insert own generation" ON public.generations
  FOR INSERT WITH CHECK (user_id IS NULL);

CREATE POLICY "select anonymous generation" ON public.generations
  FOR SELECT USING (user_id IS NULL);

CREATE POLICY "claim anonymous generation" ON public.generations
...
CREATE POLICY "anon insert items via anonymous parent" ON public.generation_items
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM generations g
      WHERE g.id = generation_items.generation_id AND g.user_id IS NULL
    )
  );

CREATE POLICY "select items via anonymous parent" ON public.generation_items
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM generations g
      WHERE g.id = generation_items.generation_id AND g.user_id IS NULL
...
-- 댓글 좋아요 정책은 supabase/community-comment-features.sql 에 있습니다.

-- 신고 ----------------------------------------------------------------------
-- 신고자는 본인 신고만 보고, 관리자(profiles.role = 'admin')는 전체를 봅니다.
-- 정의는 supabase/community-reports.sql 에 있습니다.

CREATE POLICY "community reports insert own" ON public.community_reports
  FOR INSERT TO authenticated
  WITH CHECK (reporter_id = auth.uid());

CREATE POLICY "community reports read own or admin" ON public.community_reports
...
-- 게시글 본문 이미지 버킷. public: true, 용량 제한 5MB.
-- 파일 경로는 {user_id}/... 형태이며 정책 정의는 supabase/post-images-storage.sql 에 있습니다.
--
-- [주의] SELECT 정책이 반드시 필요합니다.
-- 없으면 글 삭제 시 remove() 가 대상 객체를 찾지 못해 이미지가 남는데,
-- Storage 는 RLS 에 막혀도 error 없이 빈 결과를 주므로 조용히 실패합니다.
INSERT INTO storage.buckets (id, name, public)
...
--
-- 이후 추가한 함수·트리거·인덱스·Storage 정책은 아래 파일에 정의돼 있습니다.
--
--   supabase/post-images-storage.sql       post-images 버킷 설정과 정책
--   supabase/community-delete-cascade.sql  delete_comment_tree / delete_post_cascade RPC,
--                                          sync_post_comment_count 트리거
--   supabase/community-comment-features.sql comment_likes 테이블·정책
--   supabase/community-reports.sql         community_reports 테이블·정책,
--                                          validate_community_report_target 트리거,
--                                          is_community_admin(), review_community_report RPC,
--                                          status·target 인덱스
--   supabase/generation-rate-limit.sql     앨런 호출 레이트리밋
-- =============================================================================