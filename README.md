...
- AI 생성이 실패해도 기본 콘텐츠로 대체해 화면이 비지 않습니다
...
앨런 호출이 실패해도 화면이 빈 채로 남지 않도록, `default_contents` 에서 대체 결과를 내보냅니다.

- 형식(`format_code`)과 활성 상태로 조회한 뒤, 요청 레벨에 가까운 순으로 정렬해 3건 선택
- 조건 조합이 252가지라 상황·분위기까지는 맞추지 않습니다. **폴백의 목적은 화면 유지**이므로
  정확도보다 즉시 응답을 우선했습니다
- 폴백으로 나간 경우에도 `generations` 에 행이 남아 레이트리밋 횟수에 포함됩니다

기본 콘텐츠 367건이 미니게임 소재이자 이 폴백의 재고 역할을 합니다.
